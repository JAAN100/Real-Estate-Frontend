  ...
  const handleSubmit = async (e)=>{
    ...
    try {
      ...
      if(data.success == false){
        setImageUploadErr("Failed to create listing: " + data.message);
        return;
      }
    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4' action="">
        ...
        <div className='flex flex-col flex-1 gap-5 mt-5 mx-5'>
          ...
          <button disabled={dataUploading || isUploading} className="p-3 uppercase text-white bg-slate-700 rounded-lg hover:opacity-95 disabled:opacity-80">{dataUploading ? "Creating Listing..." : "Create Listing"}</button>
            {successMessage && <p className='text-green-500 text-center'>Listing created successfully</p>}
        </div>
       </form>
    </main>
  )
